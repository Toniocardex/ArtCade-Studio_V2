...
| ID | Work | Primary files | Done when |
|----|------|---------------|-----------|
| 3.2a | `scene.reactivate()` syncs activation; `scene.restart()` restores authored layout | `scene-lifecycle-service.cpp`, `entity-api.cpp`, `runtime-entity-gateway.cpp` | Reactivate does not recreate entities; restart calls `restoreSceneFromAuthoring` |
