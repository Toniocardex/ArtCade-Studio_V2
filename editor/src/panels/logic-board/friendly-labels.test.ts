...
describe('friendly-labels', () => {
  it('uses plain trigger names', () => {
    ...
    expect(triggerDisplayName('onMessage')).toBe('Event Bus event')
    expect(triggerDisplayName('onDialogMessage')).toBe('Dialog message')
    expect(actionDisplayName('emitEvent')).toBe('Broadcast event')
