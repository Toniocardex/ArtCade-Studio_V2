...
describe('ConsolePanel', () => {
  ...
  it('shows every project-health diagnostic together with runtime logs', () => {
    ...
    expect(text).toContain('Start scene "missing-scene" does not exist.')
    expect(text).toContain('mainScriptPath must be a non-empty project-relative path.')
    expect(text).toContain('Runtime failure.')
    expect(screen.getByRole('button', { name: /Errors 7/ })).toBeTruthy()
